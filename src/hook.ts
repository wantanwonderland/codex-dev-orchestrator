...
if (input.hook_event_name === "SubagentStart" || input.hook_event_name === "SubagentStop") {
  ...
  if (result.assignment) {
    const action = input.hook_event_name === "SubagentStop"
      ...
      : `CDO assignment ${result.assignment.id} is active for ${result.assignment.operationKey}. Produce ${result.assignment.expectedKind} at ${result.assignment.outputPath} and include the assignment metadata in cdo/v1 front matter.`;
