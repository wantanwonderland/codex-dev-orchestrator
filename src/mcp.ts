...
server.registerTool("persist_workflow_artifact_bundle", { description: "Atomically validate a complete workflow artifact handoff before writing its Markdown files into the bound worktree.", inputSchema: { projectRoot: z.string(), workflowId: z.string(), artifacts: z.array(z.object({ relativePath: z.string(), markdown: z.string() })).min(1) }, annotations: additive }, async (input) => text({ paths: await persistWorkflowArtifactBundle(input.projectRoot, input.workflowId, input.artifacts) }));
