...
4. On child return, persist planner/reviewer Markdown verbatim with `persist_workflow_artifact` rather than a generic write tool, explicitly bind `stop` if the lifecycle hook did not, then call `reconcile_agent_assignment`. A SubagentStop event is only a handoff notification; success comes from artifact, Git, lease, and state validation.
